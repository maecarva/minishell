...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/28 13:12:00 by ebonutto          #+#    #+#             */
/*   Updated: 2025/01/29 16:19:26 by ebonutto         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	execute_command(char **envp, char *cmd)
{
	...
	i = 0;
	if (ft_only_space(cmd) == 1)
		ft_error("command not found: ", ERROR_COMMAND);
	while (envp[i] != NULL && ft_strncmp(envp[i], "PATH=", 5) != 0)
		i++;
	if (envp[i] == NULL || envp[i][0] == '\0' || cmd[0] == '/' || cmd[0] == '.')
		handle_path(&cmds, &path_cmd, cmd);
	else
	{
		paths = ft_split(envp[i] + 5, ':');
		if (!paths)
			ft_perror("malloc", ERROR_CODE);
		no_path(&cmds, &path_cmd, &paths, cmd);
	}
